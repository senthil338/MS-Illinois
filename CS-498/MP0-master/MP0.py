...
def sortFreqDict(freqdict):
    ...
    #aux.sort()
    #aux.reverse()
    #aux = sorted(freqdict.items(), key=lambda x: (-x[1], x[0]))
    print freqdict["highway"]
    aux = list(sorted(freqdict.items(), key = lambda i : (i[1],i[0]), reverse = True))
    return aux
...
def process(userID):
    ...
    stop = removeStopwords(word_list,stopWordsList)
    dictionary = wordListToFreqDict(stop)
    sorteddict = sortFreqDict(dictionary)
    
    for s in sorteddict[:25]: print(str(s))
    ...
    ret = [word[0] for word in sorted_count[:20]] 
    #for word in ret:
        #print(word)
