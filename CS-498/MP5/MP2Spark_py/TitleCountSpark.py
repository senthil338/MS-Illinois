...
#counts = lines.flatMap(lambda x: re.split(r"[ \t,;\.\?!-:@\ [\]\(\){}_\*/]+", x)).filter(lambda x: x.lower() not in stop_words and len(x) > 0).map(lambda x: (x, 1)).reduceByKey(lambda x,y: x+y)
#counts.saveAsTextFile("test")



one_RDD = lines.flatMap(lambda x: unicode_to_clean_str(x))
one_RDD = one_RDD.filter(lambda x: x.lower() not in stop_words and len(x) > 0)
...
#result_1 = sorted(result, key=lambda x: (x[1], -int(x[0])))
result_1 = sorted(result, key =  lambda kv:(kv[1],kv[0]))
sc.stop()
#TODO
...
for i in range(len(result_1)):
    #print(result_1[i])
    #outputFile.write(result_1[i])
    outputFile.write(str(result_1[i][1]) + "    " + str(result_1[i][0]) +"\n")
outputFile.close()
#TODO
